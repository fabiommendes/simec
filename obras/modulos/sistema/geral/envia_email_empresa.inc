...
include_once APPRAIZ . "obras/classe/controller/OSController.class.inc";
...
if ($_REQUEST['submeter'] == 'ok'){
	...
	// Envia email
	  $sql="SELECT 
			...
			LEFT JOIN 
				obras.situacaoos so ON so.stoid = os.stoid 
			LEFT JOIN 
				obras.grupodistribuicao gd ON gd.gpdid = os.gpdid AND gpdstatus = 'A' 
			LEFT JOIN 
				obras.empresacontratada ec ON  ec.epcid = gd.epcid 
			LEFT JOIN 
				entidade.entidade e ON ec.entid = e.entid
			WHERE 
				os.orsid ='".$_REQUEST['orsid']."'";
	  ...
	  $resu =  $db->carrega_registro($RSu,0);
		  if(is_array($resu)) foreach($resu as $k=>$v) ${$k}=$v;
			  ...
			  $obOS->ativaDadosOrdemServico( array("orsid", "arqid"), $_REQUEST['orsid'] );
			  ...
			  $name    = $obOS->arqnome . '.' . $obOS->arqextensao;
				...
				if ( file_exists($caminho . $name) ){
					...
					$textoInicio = ' <b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Segue nova Ordem de Serviço que foi gerada nesta data. O documento original será enviado via SEDEX.</b>&nbsp;<br>&nbsp;<br>&nbsp;<br>&nbsp;<br> '; 
					...
					$mensagem = "Não foi encontrada a OS.";
				}
...
<html>
	...
	<body bgcolor="#ffffff" leftmargin="0" rightmargin="0" topmargin="0" bottommargin="0" marginheight="0" marginwidth="0">
		<form id="formulario" name="formulario" method="post" action="">
			...
			<input type=hidden name="orsid" value='<?=$_REQUEST['orsid']?>'>
			     ...
			     <tr>
				  ...
				  <tr>
			        <td align="right" class="subtitulodireita">Assunto:</td>
			         <?$assunto = 'SAA - Subsecretaria de Assuntos Administrativos - OS n°:'.$_REQUEST['orsid'];?>
			...
			if ( file_exists($caminho . $name) ){
				$textoInicio = ' <br><b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Segue nova Ordem de Serviço que foi gerada nesta data. O documento original será enviado via SEDEX.</b>&nbsp;<br>&nbsp;<br>&nbsp;<br>&nbsp;<br> '; 
				$conteudo = file_get_contents( $caminho . $name );
				$textoFim = '<br><br><b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Atenciosamente,<br><p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Equipe de Monitoramento de Obras</b>';
//				echo file_get_contents( $caminho . $name );
				echo $textoInicio.$conteudo.$textoFim;	
			}else{
				echo "<table align=\"center\"><tr><td><font style='color:red;'>Não foi encontrada a OS.</font></td></tr></table>";
				$osNaoEmitida = '<font style=\'color:red;\'>Não foi encontrada a OS.</font>';	
			}	
			...
			<table width='100%' align='center' border="0" cellspacing="1" cellpadding="3" align="center" style="border: 1px Solid Silver; background-color:#f5f5f5;">
			 	 <tr>
					 <td colspan="2" align="right" class="subtitulodireita">
					 	<input type="submit" class="botao" value='Enviar E-mail' onclick="envia_email_empresa(<?=$_REQUEST['orsid']; ?>)">
					 	&nbsp;&nbsp;&nbsp;
					 	<input type='button' class="botao" value='Fechar' onclick="fechar_janela(<?=$_REQUEST['orsid']; ?>)">
					 </td>
				 </tr>
			</table>
			<input type=hidden name="osNaoEmitida" value="<?=$osNaoEmitida?>">
...
<script>
    ...
    function envia_email_empresa()
  {
	...
	if (!validaBranco(document.formulario.email, 'Texto da Mensagem')) return tinyMCE.execCommand('mceFocus', true, 'osNaoEmitida');
