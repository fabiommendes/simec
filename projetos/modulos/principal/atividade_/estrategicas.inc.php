...
foreach ($anos as $key => $ano) {	
	$menu[$key] = array("descricao" => "".$ano['rofano']."", "link"=> "/projetos/projetos.php?modulo=principal/atividade_/estrategicas&acao=E&prgano=".$ano['rofano']."");
}
echo montarAbasArray($menu, "/projetos/projetos.php?modulo=principal/atividade_/estrategicas&acao=E&prgano=".$prgano."");
...
$_REQUEST['colunas'] = array("data","conclusao","estado","rofdotori","rofautorizado","rofempenhado","rofliquidado","rofpago");
$_REQUEST['execucao'] = "-1";
$_REQUEST['restricao'] = "";
...
// ----- monta parametros para a requisição
...
if ( $atiidraiz )
{
	...
	// profundidade é relativa ao item raiz
	if ( $niveisvisiveis > -1 )
	{
...
if ( in_array( 'descricao', $colunas ) )
{
	...
	if ( in_array( 'responsavel', $colunas ) )
	{
		...
		// jeito novo de se verificar quem é o gerente
		array_push(
			$join,
				"
...
$sql = <<<EOT
	select
		...
		max( coalesce( dotacaopi, 0 ) ) / 1 as rofdotori
		$campos
	from projetos.f_dadostodasatividades() da
		 inner join projetos.atividade a on a.atiid = da.atiid		 
		 left JOIN projetos.planointernoatividade pla ON pla.atiid = a.atiid and pla.pliorigem = '$pliorigem'
		 LEFT JOIN projetos.planointerno pl on pl.pliid = pla.pliid
		 left join monitora.acao aca on aca.acaid = pl.acaid
		 left JOIN financeiro.execucao exe ON exe.ptres = pl.pliptres and exe.plicod = pl.plicod and exe.rofano = '$prgano'
		$join
	$where
	group by
	 	da.numero,
...
foreach ( $colunas as $coluna )
{
	switch ( $coluna )
	{
		...
		case 'responsavel':
			...
			array_push( $label_coluna, 'Descrição' );
			break;
		case 'meta':
		...
		case 'interface':
			...
			array_push( $label_coluna, 'Órgãos Participantes' );
			break;
		case 'conclusao':
		...
		case 'data':
			...
			array_push( $label_coluna, 'Início' );
			array_push( $ordem_coluna, 'atidatafim' );
			array_push( $label_coluna, 'Término' );
			break;
		case 'estado':
			...
			break;
		case 'rofdotori':
			array_push( $ordem_coluna, 'rofdotori' );
			array_push( $label_coluna, 'Dotação Original' );
			break;							
		case 'arquivo':
		...
		case 'orcamento':
			...
			{
				array_push( $label_coluna, 'Orçamento ' . $orcamento_inicio . '-' . $orcamento_fim );
			}
			else for ( $ano = $orcamento_inicio; $ano <= $orcamento_fim; $ano++ )
			{
				array_push( $label_coluna, 'Orçamento ' . $ano  );
			}
			break;
		/*case 'rofautorizado':
			array_push( $ordem_coluna, 'rofautorizado' );
			array_push( $label_coluna, 'Lei + Créditos <BR>(Autorizado)' );
			break;*/
		case 'rofempenhado':
				...
				<tbody>
									...
									<?php
										...
										switch ( $coluna )
										{
											case 'atidescricao':
												...
												$conteudo = '<img src="../imagens/star.gif" border="0" align="top" title="Estratégica"/> <a href="/projetos/projetos.php?modulo=principal/planotrabalho/cadastro_pi&acao=A&atiid=' .$item['atiid']. '" >' . $item['numero'] . ' - ' . $item['atidescricao'] . '</a>';
												break;
											case 'atidatafim':
					...
					<?php endforeach; ?>
						<TR style="background:#DFDFDF;">
							...
							<td align="right" style="color: rgb(0, 102, 204);"><?=number_format($tRofdotori,2,',','.') ?></td>							
						...
						<TR style="background:#FFFFFF;">
							<TD colspan="10" align="right" style="font-weight:bold; font-size:9px; border-top:2px solid black; border-bottom:2px solid black;"><div style="float:left; font-size:11px;">Total de registros: <?=$total?></div><?= $data ? "* Dados financeiros atualizados até: $data" : "";?></TD>
 						</tr>
				</tbody>
			</table>
		<?php else : ?>
			<div style="color:#902020;font-size:12pt;text-align:center;">
				<br/><br/><br/>
				Não foram encontradas atividades com o filtro desejado.
			</div>
		<?php endif; ?>
